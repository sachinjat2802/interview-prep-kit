/**
 * @file sm2.ts
 * @description SuperMemo-2 (SM-2) Spaced Repetition Algorithm Implementation.
...
export function calculateSM2(quality: number, currentState?: Partial<SM2State>): SM2State {
  ...
  // New Ease Factor calculation: EF' = EF + (0.1 - (5 - q) * (0.08 + (5 - q) * 0.02))
  ...
  if (q < 3) {
    // Rating < 3 resets consecutive repetition streak
    newRepetitions = 0;
    newInterval = 1;
  } else {
